...
class Admin extends Component {
    ...
    addPostAction = (event) => {
        ...
        console.log('adding post...', { title, desc, content, published, thumbnailUrl, categories });
        this.props.createNewBlogPost({ title, desc, content, published, thumbnailUrl, categories });
        this.toggle('');
    }

    deletePostAction = (guest) => {
        this.props.removeBlogPost(guest.id);
        this.toggle('');
        this.props.fetchAdminBlogPosts(1, this.state.postsPerPage);
    }

    updatePostAction = (guest) => {
        this.props.removeBlogPost(guest.id);
        this.toggle('');
    }
